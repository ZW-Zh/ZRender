...
/////////////////////////////////////////////////////////////////////////////////
template<typename T>
vec<3, T> castVec3(vec<4,T> v) { return vec<3,T>(v.x / v.w, v.y / v.w, v.z / v.w); }
...
struct VertexIndex
{
	//顶点坐标，纹理坐标在face中的下标,顶点法线
	int v, t_v, norm_v;
	VertexIndex(int v, int t_v, int norm_v) : v(v), t_v(t_v), norm_v(norm_v) {};
};
struct Vertex
{
	//屏幕顶点坐标，世界坐标
